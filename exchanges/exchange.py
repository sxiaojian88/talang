...
class Exchange:
    # 现货API
    okexSpot = okex_util.getOkcoinSpot()
    # 期货API
    okexFuture = okex_util.getOkcoinFuture()
    ...
    def __init__(self, instmt):
        ...
        self.exchange_name = instmt.exchange_name
        self.instmt_name = instmt.instmt_name
        self.instmt_code = instmt.instmt_code
        self.base_coin = instmt.base_coin
        self.quote_coin = instmt.quote_coin

    #交易所
    #exchangename
    #上下文instrument:定价币种,基准币种,交易类型（行情,obverser,trade,strategy)


    def GetTicker(self):
        if(self.exchange_name == 'okex'):
            return self.okexSpot.ticker(symbols.format_symbol(self.base_coin, self.quote_coin, self.exchange_name))
        return
