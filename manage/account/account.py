...
import talang.data.quote.exchange_quote as ex_quote
...
class account():
    ...
    def get_btc_value_by_symbol(self, exchange, symbol):
        ...
        if symbol.lower() == ut.USDT_COIN.lower():
            value_usdt = ex_quote.exchange_quote.get_buy_1_value(self,exchange, ut.BTC_COIN.upper(), symbol.upper())
            if value_usdt != 0:
                value_btc = 1/float(value_usdt)

        else:
            value_btc=ex_quote.exchange_quote.get_buy_1_value(self,exchange, symbol.upper(), ut.BTC_COIN.upper())
    ...
    def get_usdt_value_by_symbol(self,exchange,symbol):
        value_usdt = 1.0
        if symbol.lower() != ut.USDT_COIN.lower():
            value_usdt = ex_quote.exchange_quote.get_buy_1_value(self,exchange, symbol.upper(), ut.USDT_COIN.upper())
    ...
    def get_account_okex(self):
        ...
        for i in range(0, len(keys)):
            if float(free[keys[i]]) > 0 or float(freezed[keys[i]]) > 0 or float(borrow[keys[i]]) > 0 :
                ...
                frd=freezed[keys[i]]
                bor=borrow[keys[i]]
                total=float(fre)+float(frd)+float(bor)
                ...
                t_btc_value=total*btc_value
                t_usdt_value=total*usdt_value
