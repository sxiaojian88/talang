...
class GreedMark(ModelBase):
    def __init__(self):
        ...
        self.loss_scope = -0.2                  # 强平止损幅度，-0.2表示亏损20%时候强平止损
        self.profit_scope = 0.2                 # 最低盈利幅度，超过此盈利幅度才考虑止盈强平
        self.profit_liquidation_scope = 0.2     # 盈利情况下，强平回调比例，0.2表示比盈利最高点回调20%的时候强平止盈
    ...
    def columns():
        ...
        """
        return ['contract_id', 'exchange', 'time',
                'loss_scope', 'profit_scope', 'profit_liquidation_scope',
    ...
    def types():
        ...
        """
        return ['text', 'varchar(20)', 'varchar(25)',
    ...
    def values(self):
        ...
        """
        return [self.contract_id] + [self.exchange] + [self.time] + \
               [self.loss_scope] + [self.profit_scope] + [self.profit_liquidation_scope] + \
    ...
    def print_detail(self, i=1):
        ...
        if i == 1:
            print('=' * total_with)
            format_tile = "%-10s%-20s%-20s" \
                          "%15s%15s%15s" \
                          "%15s%15s%15s%15s" \
                          "%15s%20s%20s%20s"
            print(format_tile % ("No.", "contract_id", "time",
                                 "loss_scope", "profit_scope", "liquid_scope",
                                 "top_profit", "bot_profit", "now_profit", "last_profit",
                                 "liquid_mark", "mark_time", "liquid_comp", "comp_time"
                                 ))
            print('-' * total_with)

        format_value = "%-10d%-20s%15s" \
                       "%15.4f%15.4f%15.4f" \
                       "%15.4f%15.4f%15.4f%15.4f" \
                       "%15s%25s%15s%25s"
        print(format_value % (
            i, self.contract_id, self.time,
            self.loss_scope, self.profit_scope, self.profit_liquidation_scope,
...
class GreedMarks:
    ...
    def select_greedmark_from_database(self, contract_id):
        greed_mark_get = GreedMark()
        sqlcon = "contract_id='%d'" % contract_id
        #print(sqlcon)
        ...
        for obj in select_result:
            ...
            greed_mark.loss_scope = float(obj[3])
            greed_mark.profit_scope = float(obj[4])  # 最低盈利幅度，超过此盈利幅度才考虑止盈强平
            greed_mark.profit_liquidation_scope = float(obj[5])  # 盈利情况下，强平回调比例，0.2表示比盈利最高点回调20%的时候强平止盈

            greed_mark.last_profit_loss_ratio = float(obj[6])  # 最近的上一次查询的盈亏比率
            greed_mark.now_profit_loss_ratio = float(obj[7])  # 现在查询的“盈亏比率”
            greed_mark.top_profit_loss_ratio = float(obj[8])  # 表示“盈亏比率”曾经达到的最高点
            greed_mark.bottom_profit_loss_ratio = float(obj[9])  # 表示“盈亏比率”曾经达到的最低点

            greed_mark.liquidation_mark = obj[10]  # 强平标志,默认是'NO'，强平信号出现时候标'YES',标'YES'之后就要执行强平，不再回退到'NO'状态
            greed_mark.liquidation_mark_time = obj[11]  # 统计时点时间       # 标志强退的时间点
            greed_mark.liquidation_complete = obj[12]  # 跟踪是否已经完成了强平交易
            greed_mark.liquidation_complete_time = obj[13]  # 统计时点时间   # 强平交易完成时间点
            return greed_mark
        return None
    ...
    def get_greedmark(self, contract_id):
        for greedmark in self.GreedMarks_list:
            if greedmark.contract_id == contract_id:
                return greedmark
        return None
